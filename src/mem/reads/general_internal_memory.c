...
uint16_t bios_read16(struct GBA_Core* gba, uint32_t addr)
{
    (void)gba;
    if (addr <= BIOS_END)
    {
...
uint32_t bios_read32(struct GBA_Core* gba, uint32_t addr)
{
    (void)gba;
    if (addr <= BIOS_END)
    {
...
uint8_t ewram_read8(struct GBA_Core* gba, uint32_t addr)
{
...
uint16_t ewram_read16(struct GBA_Core* gba, uint32_t addr)
{
    if (addr >= EWRAM_BEGIN && addr <= EWRAM_END)
    {
...
uint32_t ewram_read32(struct GBA_Core* gba, uint32_t addr)
{
    if (addr >= EWRAM_BEGIN && addr <= EWRAM_END)
    {
...
uint8_t iwram_read8(struct GBA_Core* gba, uint32_t addr)
{
...
uint16_t iwram_read16(struct GBA_Core* gba, uint32_t addr)
{
    if (addr >= IWRAM_BEGIN && addr <= IWRAM_END)
    {
...
uint32_t iwram_read32(struct GBA_Core* gba, uint32_t addr)
{
    if (addr >= IWRAM_BEGIN && addr <= IWRAM_END)
    {
...
uint8_t io_read8(struct GBA_Core* gba, uint32_t addr)
{
    ...
    switch (addr)
    {
        ...
        case 0x4000208 + 3: return 0xFF; // not used

        default:
            GBA_log_fatal("[IO-READ 8] addr: 0x%04X\n", addr);
            break;
    }
...
uint16_t io_read16(struct GBA_Core* gba, uint32_t addr)
{
    GBA_log("[IO-READ 16] addr: 0x%04X\n", addr);

    const uint8_t hi = io_read8(gba, addr + 0);
    const uint8_t lo = io_read8(gba, addr + 1);

    return (lo << 8) | hi;
}

uint32_t io_read32(struct GBA_Core* gba, uint32_t addr)
{
    GBA_log("[IO-READ 32] addr: 0x%04X\n", addr);

    const uint32_t hi_word_hi = io_read8(gba, addr + 0);
    const uint32_t hi_word_lo = io_read8(gba, addr + 1);
    const uint32_t lo_word_hi = io_read8(gba, addr + 2);
    const uint32_t lo_word_lo = io_read8(gba, addr + 3);
